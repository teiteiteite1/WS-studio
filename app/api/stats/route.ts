...
export async function GET(request: Request) {
  ...
  try {
    const r = await fetch(SUPABASE_URL+'/rest/v1/rpc/ws_insights_dashboard', { method:'POST', headers:{apikey:SUPABASE_KEY,Authorization:authorization,'Content-Type':'application/json'},body:JSON.stringify({p_start:p.get('all')==='1'?null:p.get('start'),p_end:p.get('end'),p_site:p.get('site') || 'official'}),cache:'no-store',signal:AbortSignal.timeout(15000) });
