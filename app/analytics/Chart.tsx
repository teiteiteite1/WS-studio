...
export default function Chart({points,start,end,label,gaps=false,gapDays=1}:{points:{day:string;value:number|null}[];start:string;end:string;label:string;gaps?:boolean;gapDays?:number}) {
 ...
 let path='';data.forEach((p,i)=>{path+=(i===0||(gaps&&Date.parse(p.day)-Date.parse(data[i-1].day)>gapDays*86400000)?'M':'L')+x(p.day)+','+y(p.value)+' ';});
