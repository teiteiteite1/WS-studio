...
export default function BriefClient() {
  ...
  return (
    <main className="brief-shell">
        ...
        <div className="brief-tabs" role="tablist" aria-label="カテゴリ">
          ...
          {(Object.keys(labels) as Category[]).map((category) => (
            <button key={category} className={active === category ? "active" : ""} onClick={() => setActive(category)}>
              {labels[category]}
              <span>{data?.items.filter((item) => item.category === category).length ?? limits[category]}</span>
            </button>
          ))}
        </div>
      ...
      {data && (
        <div className="brief-generated">UPDATED {new Date(data.generatedAt).toLocaleString("ja-JP", { timeZone: "Asia/Tokyo", month: "numeric", day: "numeric", hour: "2-digit", minute: "2-digit" })}</div>
      )}
      ...
      <section className="brief-list">
        {loading && !data ? (
          <div className="brief-empty">今日の情報を集めています…</div>
        ) : visible.length === 0 ? (
          <div className="brief-empty">ここは空っぽ。今日は静かな日です。</div>
        ) : (
          visible.map((item) => {
            ...
            return (
              <article key={item.id} className={`brief-card ${isRead ? "is-read" : "is-unread"}`}>
                ...
                <div className="brief-meta">
                  ...
                  <span>{new Date(item.publishedAt).toLocaleDateString("ja-JP", { timeZone: "Asia/Tokyo", month: "short", day: "numeric" })}</span>
                </div>
      ...
      <footer className="brief-footer">
        <p>上限：AI 5 / 泌尿器科 3 / 透析 3。重要なものがなければ無理に埋めません。</p>
      </footer>
    </main>
  );
}
