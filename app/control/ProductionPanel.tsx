...
function stockOf(i: ShafuIdea) { return i.stockStatus || (i.status === "dismissed" ? "hold" : i.status === "in_progress" ? "prompted" : "unused"); }
...
export default function ProductionPanel({ view, setView, workspace, mutate, callAI, ready, notify }: Props) {
    ...
    const [ideaFilter, setIdeaFilter] = useState<"all" | "unused" | "video">("all");
    const visibleIdeas = workspace.ideas.filter(i =>
        (ideaFilter === "all" || (ideaFilter === "video" ? stockOf(i) === "video" : stockOf(i) !== "video")) &&
        [i.body, i.title, ...i.tags].join(" ").toLowerCase().includes(query.toLowerCase()));
 ...
 {view === "ideas" && <><div className="idea-status-filters" role="group" aria-label="ネタのステータスで絞り込み">{([["all", "すべて"], ["unused", "未使用"], ["video", "動画化済み"]] as const).map(([value, label]) => <button type="button" key={value} aria-pressed={ideaFilter === value} onClick={() => setIdeaFilter(value)}>{label}</button>)}</div>{ideaFilter === "unused" && <p className="idea-filter-hint">動画化前のネタを表示（プロンプト化済み・保留を含む）</p>}<div className="production-toolbar"><input aria-label="ネタ検索" type="search" placeholder="本文・タグで検索" value={query} onChange={e => setQuery(e.target.value)}/><button onClick={() => setDraft(idea())}>＋ 新規追加</button></div>{draft && <article className="production-editor"><label>ネタ本文<textarea rows={5} value={draft.body} onChange={e => setDraft({ ...draft, body: e.target.value })}/></label><label>タグ（カンマ区切り）<input value={draft.tags.join(",")} onChange={e => setDraft({ ...draft, tags: e.target.value.split(/[,、]/) })}/></label><label>ステータス<select value={draft.stockStatus || "unused"} onChange={e => setDraft({ ...draft, stockStatus: e.target.value as ShafuIdea["stockStatus"] })}>{Object.entries(stocks).map(([k, v]) => <option key={k} value={k}>{v}</option>)}</select></label><button disabled={!draft.body.trim()} onClick={() => { const row = { ...draft, title: draft.body.trim().slice(0, 24), body: draft.body.trim(), tags: draft.tags.map(s => s.trim()).filter(Boolean), updatedAt: now() }; mutate(w => { const n = w.ideas.findIndex(x => x.id === row.id); if (n < 0)
        w.ideas.unshift(row);
    else
        w.ideas[n] = row; }); setDraft(null); }}>保存</button><button onClick={() => setDraft(null)}>キャンセル</button></article>}<div className="production-list">{visibleIdeas.map(i => <article key={i.id}><small>{i.createdAt.slice(0, 10)} · {stocks[stockOf(i)]}</small><p>{i.body}</p><p>{i.tags.join(" / ")}</p><div className="production-toolbar"><button onClick={() => send(i)}>PROMPTへ</button><button onClick={() => { setEditing(archive(i)); setView("archive"); }}>ARCHIVEへ</button><button onClick={() => setDraft(structuredClone(i))}>編集</button><button onClick={() => { if (confirm("このネタを削除しますか？"))
        mutate(w => { w.ideas = w.ideas.filter(x => x.id !== i.id); }); }}>削除</button></div></article>)}{!visibleIdeas.length && <p role="status">{workspace.ideas.length ? "条件に一致するネタはありません。" : "思いついたネタを保存しておこう。"}</p>}</div></>}
