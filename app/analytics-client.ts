import { safeUrl, sourceFor, isPublicPath, isPublicTarget } from './lib/telemetry';
...
async function flush() {
  ...
  await Promise.all(Array.from(pending,async ([id,body])=> {
    if (!isPublicPath(String(body.path || ''))) { pending.delete(id); persist(); return; }
...
export function trackEvent(kind:string, value?:string, target?:string) {
  if (typeof window === 'undefined' || !isPublicPath(location.pathname) || !isPublicTarget(target) || (document.referrer && !isPublicTarget(document.referrer)) || navigator.webdriver || storageGet('ws_analytics_exclude') === '1') return;
