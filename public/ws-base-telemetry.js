...
(() => {
 const site='base';
 if(location.hostname!=='wsstudiotei.base.shop'||/^\/(cart|checkout|mypage|account|admin)(\/|$)/i.test(location.pathname)||window.__wsTelemetryReady)return;
 const publicLink=value=>{if(!value)return true;try{const u=new URL(value,location.href);if(['ws-studio-hub.wsstudio.chatgpt.site','ws-social-desk.wsstudio.chatgpt.site','now-generating-album.wsstudio.chatgpt.site'].includes(u.hostname))return false;if(u.hostname==='ws-studio-wheat.vercel.app'&&/^\/(analytics|insights?|hub|social-desk|social|control|brief|learn|ai30|ai-30|counter|admin|api)(\/|$)/i.test(decodeURIComponent(u.pathname)))return false;return true;}catch{return false;}};
 if(!publicLink(document.referrer))return;
 ...
 const track=(kind,value,target)=>{
  ...
  if(/^\/(cart|checkout|mypage|account|admin)(\/|$)/i.test(location.pathname)||!publicLink(target))return;
